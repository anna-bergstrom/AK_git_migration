...
# Creating a moving average for time series plots and analysis
swindow <- 192
...
precip16$temp <- full_wx$site_temp[full_wx$local_time >= bounds16[1,] & full_wx$local_time <= bounds16[2,]]
...
precip17$temp <- full_wx$site_temp[full_wx$local_time >= bounds17[1,] & full_wx$local_time <= bounds17[2,]]
names(precip17)[1] <- "precip17int"
merged17<- merge.xts(xts(gauge_data17$Q_m3s, as.POSIXct(gauge_data17$datetime)),xts(gauge_data17$SC_filled, as.POSIXct(gauge_data17$datetime)),xts(precip17$precip17int, as.POSIXct(precip17$datetime)),xts(precip17$temp, as.POSIXct(precip17$datetime)),fill=NA)
names(merged17)<- c("Q_m3s","SC_filled", "precip17", "temp17")  
...
#hourly_norm$SC_hr_norm <- hourly_norm$SC_hr_norm * 10
...
dep_con16_dist <- dist(hourly_norm, method = "euclidean") #distance matrix
 dep16_clust <- chclust(dep_con16_dist, method = "coniss") #clustering
 bstick(dep16_clust, ng=20, plot=TRUE) #broken stick plot showing reduction in sse over n of clusters
 memb<-cutree(dep16_clust,k=5) #cutting the number of clusters off at 5 and defining the membership of those 5 clusters


# Plotting 2016 clustering results 
